...
locals {
  ...
  allow_egress_rule_ipv4 = {
    key             = "_allow_all_egress_ipv4_"
    rule_number     = local.allow_all_egress_ipv4_rule_number
    type            = "egress"
    protocol        = "-1"
    rule_action     = "allow"
  ...
  allow_egress_rule_ipv6 = {
    key             = "_allow_all_egress_ipv6_"
    rule_number     = local.allow_all_egress_ipv6_rule_number
    type            = "egress"
    protocol        = "-1"
    rule_action     = "allow"
    cidr_block      = null
  ...
  extra_rules = local.allow_all_egress ? [local.allow_egress_rule_ipv4, local.allow_egress_rule_ipv6] : []
  ...
  # nacl_exploded_rules = flatten([for rule in local.nacl_rules_lists : {
  #   key = "${rule.key}"
  ...
  all_resource_rules   = concat(local.norm_rules, local.self_rules, local.other_rules, local.extra_rules)
